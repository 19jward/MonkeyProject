...
### 5 behaviors
- yeah none of this works past 3 behaviors :)
...
fake_behaviors5.0 <- c("move", "feed", "aggresive", "vigilance", "move", "sleep", "vigilance",
                     "feed", "sleep", "move", "sleep", "move", "aggresive", "vigilance",
                     "sleep", "move", "feed", "move", "feed", "aggressive", "move",
                     "aggressive", "move", "feed", "move", "sleep", "move", "feed", "aggresive", "vigilance",
...
test5 <- fit_MM(fake_data5.0, fake_behaviorlist5)
test5

## accuracy check 
tm5 <- matrixfunct(fake_data5.0, fake_behaviorlist5)
tm5
```
i broke it??? i thought this issue was resolved idk whats up here?????? like the original function not the modified one later :(
...
## VI.III optimizing (replacing fit_MM)
- adds a condition for by row or by whole matrix 

- this was a flop week ngl

- currently running into issues i think with like naming??? its not working somewhere bc of choosing the singular row i need for the P
```{r}
  ...
  if(!isTRUE(rowwise)){
    ...
    for(b1 in behavior_list){
      fit <- optim(par = par, fn = function(par, data){
        ...
        if(any(par > 1)) return(-Inf)
        Pf <- matrix(par, nrow = length(behavior_list)) 
          ## this being the full matrix here makes me feel like im not actually optimizing by row
        if(any(rowSums(Pf) > 1)) return(-Inf)
        Pf <- cbind(Pf, 1 - rowSums(Pf))
        rownames(Pf) <- colnames(Pf) <- behavior_list
        Pf <- Pf[b1, ] ## this is where its not working rn, is it bc b1 is a string not a number??
                       ## but it is the string that is the name of the row so idk 
        get_ll_row(X, Pf, row_behavior = b1, behavior_list)}, data = data, ## not sure about the b1 here 
        control = c(fnscale = -1, maxit = maxit))
   
       row_out <- fit$par
       row_out <- cbind(row_out, 1 - rowSums(row_out))
       rownames(row_out) <- behavior_list 
       colnames(row_out) <- behavior_list
       
       out <- rbind(out, row_out)
       
    }
    
    rownames(out) <- colnames(out) <- behavior_list ## this is where its having issues???? why????
    return(out)
  }
}
```
