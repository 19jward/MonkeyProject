...
## X is an FxM matrix (indicator for covariates)
## beta is full MxBxB array
## T_max gives a stopping point for how long the focal is

## so it like works.... kinda for 1 focal only which was not the goal but i really only need 1 focal anyway
## kinda lost it and i dont think this should be this long but here we are

sim_data <- function(X, beta, T_max = 600) {
  if (is.null(dim(X))) {
    X <- matrix(X, nrow = 1)
  ...
  for (i in 1:F) {
    ...
    for (m in 1:M) {
      trans_mat <- trans_mat + x_i[m] * beta[m, , ]
    }
    
    time <- 0 ; Behavior <- c() ; TimeSpent <- c()
    ...
    while (time < T_max) {
      ...
      dt <- sample(1:150, 1)
      TimeSpent <- c(TimeSpent, dt)
      time <- time + dt
      
      probs <- trans_mat[current_behavior, ]
      probs[current_behavior] <- 0 ## dont choose the same behavior, this might mess it up??
      current_behavior <- sample(1:B, 1, prob = probs)
    } ## end while loop
    
    out[[i]] <- data.frame(FocalID = i, Behavior, TimeSpent, Habitat = 1)
  } ## end loop over focals
  ...
  out$Habitat <- replace(out$Habitat, out$Habitat == 0, "jalan")
  out$Habitat <- replace(out$Habitat, out$Habitat == 1, "hutan")
  
  return(out)
} ## end function

pls_work <- sim_data(X_test, beta_test) 

optim_cov_ll_full(pls_work, X_test, blist) ## oOF something is Wrong
                                          ## is it bc i set current behavior to 0?
```
