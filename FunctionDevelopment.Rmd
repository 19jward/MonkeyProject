...
get_ll <- function(data, P, behaviorshortlist){
...
# V. Optimizing LL
=======
>>>>>>> 25199879d63856278acd09d2338f64e4ebe1b9cc
...
## applying to our function (start with 1 term???)
```{r}
## so get_ll takes a focal and a transition matrix, we want to try to optimize just one of the transitions to start?? so it would have to be a row/1 behavior?? not 100% sure what we mean by starting with 1

## maybe i do this with a fake little dataset to see if it works (3 behaviors)
...
colnames(X) <- c("TimeSpent", "Behavior") ## match my fake one to work with the matrixfunctions i made

fake_behaviorshortlist <- c("move", "sleep", "feed")
Pf <- matrixfunct(X, fake_behaviorshortlist) ## transition matrix of fake focal

get_ll(X, Pf, fake_behaviorshortlist) ## wow great news the function works love that
...
## par is the intiial values (starting point to optimize from)
...
fit <- optim(par = Pf, fn = function(par, X){
  get_ll(X, par, fake_behaviorshortlist)}, X = fake_data, control = c(fnscale = -1))

## what a fun number of NA warnings
## try next: not using the pf
...
fit <- optim(par = initial_matrix_fake, fn = function(par, X){
  get_ll(X, par, fake_behaviorshortlist)}, X = fake_data, control = c(fnscale = -1))

## yeah thats not the problem here

## realizing idk what is actually meant by starting with one term
## ok im being silly and i think this actually needs a loop for each entry right???
