...
get_LL <- function(data, P){
  # y1 <- first listed behavior in the data
  y1 <- data$Behavior[1]
  # p1 <- probabilty of starting behavior (idk where im getting this yeet)
  
  # t <- behavior (is t our index?)
  # yt <-  vector of 0s and 1s that tell us what behavior we are at (idk how to this)
...
Qs:
- where am i getting p1 from (probability of being in the first behavior), does that come from the data or is that something we are going to preset as some constant vector for the  behaviors?
- is t the index?
- idk how to do the yt vector that tells us which behavior it is in time t w/vector of 0s and 1

-for loop to define pt?


- like somewhere there is supposed to be a part where i can put some variables like forest/road or whatever and that is included in the likelihood?
